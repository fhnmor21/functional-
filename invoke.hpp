...
namespace FunctionalCpp
{
  ...
  namespace Tuple
  {
    ...
    private:
      ...
      {
        return std::move(
            std::tuple< A2, A1, typename std::remove_reference< As >::type... >(a2, a1, as...));
      }

      template < size_t... S >
      static std::tuple< A2, A1, As... > swap_impl1(std::tuple< A1, A2, As... > from,
                                                    std::index_sequence< S... >)
      {
        return Swapper< A1, A2, As... >::swap_impl2(
            std::get< S >(std::forward< std::tuple< A1, A2, As... > >(from))...);
      }

    public:
      static std::tuple< A2, A1, As... > swap(std::tuple< A1, A2, As... > from)
      {
        size_t constexpr tSize = std::tuple_size<
            typename std::remove_reference< std::tuple< A1, A2, As... > >::type >::value;
